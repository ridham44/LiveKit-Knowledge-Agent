...
export default function OtpVerification({
  ...
  const submitVerify = async (otpValue) => {
    ...
    setResendNotice('');
    try {
      const data = await auth.verifySignupOtp(email, otpValue);
  ...
  const handleResend = async () => {
    ...
    setResendNotice('');
    try {
      const data = await auth.resendSignupOtp(email);
  ...
  return (
    <div>
      ...
      <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-50 mb-1.5">Verify your email</h2>
      ...
      {sessionInvalid ? (
        <div className="text-center py-4">
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
            Your signup session has expired. Please start signup again.
          </p>
          <button
            type="button"
            onClick={onBack}
